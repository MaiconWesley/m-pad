...
export default function Navbar({ activeView, onViewChange }: NavbarProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full bg-[#0F1115]/95 border-t border-white/10 p-4 flex justify-center gap-24 items-center text-slate-400 backdrop-blur-sm z-20">
      {navItems.map((item) => (
        <button
          key={item.id}
